...
export default function Subjects() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return list
    return list.filter(s => (s.name + ' ' + (s.className||'') + ' ' + (s.teacherName||'')).toLowerCase().includes(q))
  }, [list, query])
  ...
  function handleAdd(e) {
    ...
    const d = divisions.find(x => x.id === classId)
    const newItem = {
      id: 'sub' + Date.now(),
      name: name.trim(),
      classId: classId || '',
    ...
    setName(''); setClassId(''); setTeacherId(''); setShowForm(false)
  }
  ...
  function saveEdit(e) {
    ...
    setList(prev => {
      const updated = prev.map(it => {
        ...
        const d = divisions.find(x => x.id === editClassId)
        return { ...it, name: editName.trim() || it.name, classId: editClassId || '', className: d ? d.name : '', teacherId: editTeacherId || '', teacherName: t ? t.name : '' }
      })
        ...
        {filtered.map(s => (
          <div key={s.id} className={`bg-white shadow rounded-lg p-4 flex items-center justify-between ${s.active === false ? 'opacity-60' : ''}`}>
            <div>
              ...
              <div className="text-sm text-gray-500">Class: {s.className || '—'} · Teacher: {s.teacherName || '—'}</div>
            </div>
