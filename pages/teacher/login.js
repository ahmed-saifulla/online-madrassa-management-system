...
export default function TeacherLogin() {
  ...
  const demoLogin = () => {
    setUsername('teacher@example.com')
    ...
    <>
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="mt-6 bg-white p-6 rounded-lg shadow">
        ...
        <label className="block">
          ...
          <input type="email" value={username} onChange={e => setUsername(e.target.value)} className="mt-1 block w-full rounded-md border-gray-200 shadow-sm" placeholder="teacher@example.com" />
        </label>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="text-sm text-gray-500">Teacher access</div>
        </div>
      </form>
    </>
  )
